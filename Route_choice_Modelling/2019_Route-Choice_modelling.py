...
def read_GTFS_data(filename: str) -> None:
    ...
    trips_df = trips_df.merge(routes_df[['route_id', 'route_short_name']], on='route_id', how='left')
    stop_times_df = stop_times_df.merge(trips_df[['trip_id', 'route_short_name']], on='trip_id', how='left')
    return None
...
def stops_trips_fn():
    ...
    for route in routes_df.route_short_name.unique():
        route_name_trips_dict[route] = set(trips_df[trips_df.route_short_name == route].trip_id.unique())
    return None


def preProcessing(gtfs_path: str, file_path: str) -> None:
    """
    ...
    :return: None
    """
    global od_data, routes_df, trips_df, stop_times_df, stops_df, stops_dict
    ...
    lines_df = pd.read_excel('./Route_choice_Modelling/edited_NBT19_Definitions_Published.xlsx',
                             sheet_name='Lines', engine='openpyxl')
    ...
    od_data['route'] = od_data['route'].apply(lambda x: x.split('|'))
...
def leg_wise_attributes(orig_id: str, dest_id: str, route: str, departure_time: str) -> tuple:
    ...
    """
    global stop_times_df, stops_df, route_name_trips_dict, stops_trips_dict, leg_stops_sequence, route_name_stops_dict
    ...
    try:
        trip_id = trips[0]
        print(orig_id, dest_id, departure_time, trip_id)
...
def route_wise_attributes(route: list, dept_time: pd.to_timedelta) -> dict:
    ...
    """
    global stop_times_df, stops_df, route_name_trips_dict, stops_trips_dict, stops_dict

    route_attributes = {'IVTT': 0, 'stop_sequence': [], 'initial_wait_time': 0, 'transfer_wait_time': 0, 'Not_found': 0}
    for i in range(len(route)):

        org_id, dest_id, route_id = stops_dict[route[i][:4]]['stop_id'], stops_dict[route[i][5:9]]['stop_id'], route[i][
                                                                                                               -3:]
        IVTT, wait_time, stop_sequence, dept_time = leg_wise_attributes(org_id, dest_id, route_id, dept_time)
        if IVTT == -10000:
...
def row_wise_attributes(row: dict,dept:str) -> dict:
    """
    ...
    for alt in range(1, max_alt + 1):
        ...
        if route == 0:
            ...
            route_attributes = route_wise_attributes(route, dept)
            row_attributes[f'transfer_{alt}'] = len(route) - 1
...
if __name__ == '__main__':
    ...
    preProcessing(file_path='./Route_choice_Modelling/NBT19MTT3a_routechoice_tb_wf.csv',
                  gtfs_path='./Route_choice_Modelling/GTFS.zip')
    ...
    od_trail = od_timeInterval_dict['6']
    od_trail = od_trail.loc[od_trail.od == 'HSTu>TPLu', :].reset_index(drop=True)
    results = []
    dept_time = ['22:09:31','22:34:57','22:09:31','22:09:31','22:09:31']
    for id, row in tqdm(od_trail.iterrows(), total=od_trail.shape[0]):
        results.append(row_wise_attributes(row,dept_time[id]))
    results_df = pd.DataFrame(results)
    od_trail = pd.concat([od_trail, results_df], axis=1)
    columns_na = [f'stop_sequence_{i}' for i in range(1, max_alt + 1)]
    od_trail.loc[:, columns_na] = od_trail.loc[:, columns_na].fillna(0)
    results = []
    for id, row in tqdm(od_trail.iterrows()):
        results.append(path_size(row))
    results_df = pd.DataFrame(results)
    od_trail = pd.concat([od_trail, results_df], axis=1)
    od_trail.to_csv('./Route_choice_Modelling/od_trail.csv', index=False)
